...
- **~~Seguridad y Prevención de Baneos:~~** ✅ COMPLETADO. Implementado pool de proxies residenciales rotativos a través de la variable de entorno `PROXY_LIST` en `fetcher.ts` con rotación dinámica y tolerancia a fallos.
- **~~Autenticación sin Contraseña:~~** ✅ COMPLETADO. Magic Links integrado en el backend (AuthService/Controller) y frontend (AuthModal, App.tsx de detección en inicio). Enlaces de desarrollo logueados directamente en la terminal.
...
- **~~Sustitución Heurística de Formato:~~** ✅ COMPLETADO. Intercepción de carritos en `SubstitutionService` para sugerir alternativas de volumen más económicas del mismo producto/marca y aplicación directa en UI.
