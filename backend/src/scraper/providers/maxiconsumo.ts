...
const MAXICONSUMO_SEARCH_API = 'https://www.maxiconsumo.com.ar/api/catalog_system/pub/products/search';

export class MaxiconsumoScraper extends BaseScraper {
    constructor() {
        super('maxiconsumo');
    }
    ...
    async performScraping(): Promise<void> {
        ...
        for (const term of searchTerms) {
            console.log(`[Provider:Maxiconsumo] Buscando: "${term}"...`);

            const data = await fetchWithRetry<any[]>(`${MAXICONSUMO_SEARCH_API}?ft=${term}`);

            if (!Array.isArray(data)) {
                console.warn(`[Provider:Maxiconsumo] Respuesta inválida para ${term}`);
                continue;
            }
            ...
            for (const item of data) {
                ...
                const sourceUrl = item.link || `https://www.maxiconsumo.com.ar${item.linkText}/p`;
